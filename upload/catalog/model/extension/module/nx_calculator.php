<?php
  class ModelExtensionModuleNxCalculator extends Model {
    ...
    public function getFormulaAdv($product_id, $url) {
      ...
      $query = $this->db->query("SELECT DISTINCT nf.formula, nft.priority, nf.value, nr.url, nr.html_id FROM `".DB_PREFIX."product` AS p JOIN `".DB_PREFIX."product_to_category` AS ptc ON p.product_id = ptc.product_id JOIN `".DB_PREFIX."nxc_formula_table` AS nft ON (p.product_id = nft.table_id AND nft.table = 0) OR (ptc.category_id = nft.table_id AND nft.table = 1) OR (p.manufacturer_id = nft.table_id  AND nft.table = 2) JOIN `".DB_PREFIX."nxc_formula` AS nf ON nf.nxc_formula_id = nft.nxc_formula_id JOIN `".DB_PREFIX."nxc_route` AS nr ON nr.nxc_route_id = nft.nxc_route_id WHERE ((p.product_id = $product_id AND (nft.table = 0 OR nft.table = 2)) OR ptc.product_id = $product_id AND nft.table = 1) AND nr.url = '$url';");
      return $query->rows;
    }
  }